...
# def create_dataset(data_format):
#     data_list = []
...
class MoleculeNetGraphDataset(InMemoryDataset):
    ...
    def processed_file_names(self):
        ...
        elif self.dataset == 'FGFR1':
            smiles_list, rdkit_mol_objs, labels = \
                _load_FGFR1_dataset(self.raw_paths[0])
            data_list, data_smiles_list, data_label_list = shared_extractor(
                smiles_list, rdkit_mol_objs, labels)

        elif self.dataset == 'HPK1':
            smiles_list, rdkit_mol_objs, labels = \
                _load_HPK1_dataset(self.raw_paths[0])
            data_list, data_smiles_list, data_label_list = shared_extractor(
                smiles_list, rdkit_mol_objs, labels)

        elif self.dataset == 'FDA':
            smiles_list, rdkit_mol_objs, labels = \
                _load_FDA_dataset(self.raw_paths[0])
            data_list, data_smiles_list, data_label_list = shared_extractor(
                smiles_list, rdkit_mol_objs, labels)
...
def _load_FGFR1_dataset(input_path):
    input_df = pd.read_csv(input_path, sep=',')
    smiles_list = input_df['Drug']
    rdkit_mol_objs_list = [AllChem.MolFromSmiles(s) for s in smiles_list]
    labels = input_df['Y']
...
def _load_HPK1_dataset(input_path):
    input_df = pd.read_csv(input_path, sep=',')
    smiles_list = input_df['Drug']
    rdkit_mol_objs_list = [AllChem.MolFromSmiles(s) for s in smiles_list]
    labels = input_df['Y']
...
def _load_FDA_dataset(input_path):
...
if __name__ == '__main__':
    ...
    #
    # # train_loader = dataloader_class(train_dataset, batch_size=args.batch_size, shuffle=True,
    # #                                 num_workers=args.num_workers)
    # # val_loader = dataloader_class(valid_dataset, batch_size=args.batch_size, shuffle=False,
    # #                               num_workers=args.num_workers)
    # # test_loader = dataloader_class(test_dataset, batch_size=args.batch_size, shuffle=False,
    # #                                num_workers=args.num_workers)
    #
    #
    # # LiuC:
