...
class Batch():
    ...
    def __init__(self, idx, attn_bias, attn_edge_type, spatial_pos, in_degree, out_degree, x, edge_input,
                 text, text_mask, smiles, smiles_mask
                 ):
    ...
    def to(self):
        ...
        self.smiles = self.smiles.to(device)
        self.smiles_mask = self.smiles_mask.to(device)
        return self
...
def collator(items, max_node=512,
    ...
    idxs, attn_biases, attn_edge_types, spatial_poses, in_degrees, out_degrees, xs, edge_inputs, \
        textes, text_maskes, smileses, smiles_maskes= zip(*items)
...
'''
def collator_item(item, max_node=512, multi_hop_max_dist=20, spatial_pos_max=20):
    if item is None or item.x.size(0) > max_node:
        return None
    ...
    x = pad_2d_unsqueeze(x, max_node_num)
    edge_input = pad_3d_unsqueeze(edge_input, max_node_num, max_node_num, max_dist)
    attn_bias = pad_attn_bias_unsqueeze(attn_bias, max_node_num + 1)
    attn_edge_type = pad_edge_type_unsqueeze(attn_edge_type, max_node_num)
    spatial_pos = pad_spatial_pos_unsqueeze(spatial_pos, max_node_num)
    in_degree = pad_1d_unsqueeze(in_degree, max_node_num)
    out_degree = pad_1d_unsqueeze(out_degree, max_node_num)
