...
from MolTox.collator_prop_molBT import collator, Batch
...
def train_classification(model, device, loader, optimizer):
    ...
    for step, batch in enumerate(L):
        ...
        # LiuC:
        # train_loader_iter = iter(L)
        # first_batch = next(train_loader_iter)

        # molecule_repr = get_molecule_repr_MoleculeSTM(
        #     first_batch, mol2latent=None,
...
#
#     return {'Pearson': pearson_corr, 'Spearman': spearman_corr}, y_true, y_pred
...
def eval_regression(model, device, loader):
    ...
    # y_true = torch.cat(y_true, dim=0).cpu().numpy()
    # y_pred = torch.cat(y_pred, dim=0).cpu().numpy()
    y_true = torch.cat(y_true, dim=0).cpu().numpy().squeeze()
    y_pred = torch.cat(y_pred, dim=0).cpu().numpy().squeeze()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_model_dir", type=str, default='save_model/task4_tox/HPK1')
